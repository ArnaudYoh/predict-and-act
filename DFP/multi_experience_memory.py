...
class MultiExperienceMemory:
    ...
    def __init__(self, args, multi_simulator=None, target_maker=None):
        ...
        # TODO make the magic number disappear ?
    ...
    def reset(self):
        ...
        self._objects = my_util.make_array(shape=(self.capacity,) + self.objects_shape, dtype=np.float32,
                                           shared=self.shared, fill_val=0.)
        ...
        self._actions = my_util.make_array(shape=(self.capacity,) + self.action_shape, dtype=np.int, shared=self.shared,
                                           fill_val=0)
        self._objectives = my_util.make_array(shape=(self.capacity,) + self.obj_shape, dtype=np.float32,
                                              shared=self.shared, fill_val=0.)
        self._n_episode = my_util.make_array(shape=(self.capacity,), dtype=np.uint64, shared=self.shared,
                                             fill_val=0)  # this is needed to compute future targets efficiently
        self._n_head = my_util.make_array(shape=(self.capacity,), dtype=np.uint64, shared=self.shared,
                                          fill_val=0)  # this is needed to compute future targets efficiently
    ...
    def add(self, imgs, meass, objects, rwrds, terms, acts, objs=None, preds=None):
        ...
        Args:
            imgs: single observation frame
            meass: extra measurements from the state
            objects: extra info about object (ammo, health kit ..etc) position on screen
            rwrds: reward
            terms: terminal state
            acts: action taken
        '''
        ...
        self._objects[term_inds] = self._objects[(term_inds - 1) % self.capacity]
        ...
        self._objects[term_inds[prev_terminals]] = 0.
        # end of hack
        ...
        self._terminals[
            self._curr_indices] = True  # make the following state terminal, so that our current episode doesn't get stitched with the next one when sampling states

    def add_step(self, multi_simulator, acts=None, objs=None, preds=None):
        if acts is None:
            acts = multi_simulator.get_random_actions()
        to_add = multi_simulator.step(acts)
        self.add(*(to_add + (acts, objs, preds)))

    def add_n_steps_with_actor(self, multi_simulator, num_steps, actor, verbose=False, write_predictions=False,
                               write_logs=False, global_step=0):
        ns = 0
        last_meas = np.zeros((multi_simulator.num_simulators,) + self.meas_shape)
        last_objects = np.zeros((multi_simulator.num_simulators,) + self.objects_shape)
        if write_predictions and not hasattr(self, '_predictions'):
            self._predictions = my_util.make_array(shape=(self.capacity,) + actor.predictions_shape, dtype=np.float32,
                                                   shared=self.shared, fill_val=0.)
        ...
        if write_logs:
            ...
            accum_objects = np.zeros((multi_simulator.num_simulators,) + self.objects_shape)
            total_final_meas = np.zeros(self.meas_shape)
            total_avg_meas = np.zeros(self.meas_shape)
            total_final_objects = np.zeros(self.objects_shape)
            total_avg_objects = np.zeros(self.objects_shape)
        ...
        for ns in range(int(num_steps)):
            ...
            if write_logs:
                ...
                for ns in terminated_simulators:
                    ...
                    avg_objects = accum_objects[ns] / float(num_episode_steps[ns])
                    total_avg_meas += avg_meas
                    total_avg_objects += avg_objects
                    total_final_meas += prev_meas[ns]
                    total_final_objects += prev_objects[ns]
                    total_accum_reward += accum_rewards[ns]
                    start_times[ns] = time.time()
                    log_detailed.write(log_detailed_format.format(*(
                            [num_episodes, num_episode_steps[ns], episode_time, accum_rewards[ns]] + list(
                        prev_meas[ns]) + list(avg_meas))))
                    accum_meas[ns] = 0
                    accum_objects[ns] = 0
                    accum_rewards[ns] = 0
        ...
        if write_logs:
            if num_episodes == 0:
                num_episodes = 1
            log_brief.write(log_brief_format.format(*(
                    [global_step, time.time(), time.time() - total_start_time, num_episodes,
                     total_accum_reward / float(num_episodes)] +
                    list(total_final_meas / float(num_episodes)) + list(total_avg_meas / float(num_episodes)) +
                    list(total_final_objects / float(num_episodes)) + list(total_avg_objects / float(num_episodes)))))
            log_brief.close()
            log_detailed.close()
    ...
    def get_states(self, indices):
        ...
        for (ni, index) in enumerate(indices):
            frame_slice = np.arange(int(index) - self.history_length * self.history_step + 1, (int(index) + 1),
                                    self.history_step) % self.capacity
    ...
    def get_targets(self, indices):
        ...
        return self.target_maker.make_targets(indices, self._measurements, self._rewards,
                                              self._n_episode + 12345678 * self._n_head)

    def has_valid_history(self, index):
        return (not self._terminals[
            np.arange(int(index) - self.history_length * self.history_step + 1, int(index) + 1) % self.capacity].any())
    ...
    def has_valid_target(self, index):
        return (not self._terminals[
            np.arange(index, index + self.target_maker.min_future_frames + 1) % self.capacity].any())
    ...
    def show(self, start_index=0, end_index=None, display=True, write_imgs=False, write_video=False,
             preprocess_targets=None, show_predictions=0, net_discrete_actions=[]):
        if show_predictions:
        ...
        if write_video:
            ...
            vw = VideoWriter('vid.avi', (self.img_shape[2], self.img_shape[1]), framerate=24,
                             rgb=(self.img_shape[0] == 3), mode='replace')
        ...
        while True:
            ...
            if curr_index == start_index:
                if display or write_imgs:
                    ...
                    txt = ax_img.text(self.img_shape[2] - 10 * len(self._measurements[curr_index]),
                                      self.img_shape[1] - 5, str(self._measurements[curr_index]), fontsize=20,
                                      color='red')
                if show_predictions:
                    all_objs = np.sum(self._predictions, axis=2)
                    sbp = my_util.StackedBarPlot(curr_preds, ylim=[np.min(all_objs), np.max(all_objs)],
                                                 labels=curr_labels)
                    del all_objs
                    if display:
                        sbp.show()
