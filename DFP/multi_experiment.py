...
from .future_predictor_agent_advantage_object_info_depths import FuturePredictorAgentAdvantageObjectDepth
...
class MultiExperiment:
    ...
    def __init__(self, target_maker_args={},
        ...
        test_policy_experience_args = my_util.merge_two_dicts(defaults.test_policy_experience_args,
                                                              test_policy_experience_args)
        ...
        self.train_experience = MultiExperienceMemory(train_experience_args, multi_simulator=self.multi_simulator,
        ...
        if 'meas_for_net' in experiment_args:
            ...
            for ns in range(self.train_experience.history_length):
                agent_args['meas_for_net'] += [i + self.multi_simulator.num_meas * ns for i in experiment_args[
                    'meas_for_net']]  # we want these measurements from all timesteps
        ...
        if len(experiment_args['meas_for_manual']) > 0:
            agent_args['meas_for_manual'] = np.array(
                [i + self.multi_simulator.num_meas * (self.train_experience.history_length - 1) for i in
                 experiment_args['meas_for_manual']])  # current timestep is the last in the stack
        else:
        ...
        self.test_policy_experience = MultiExperienceMemory(test_policy_experience_args,
                                                            multi_simulator=self.multi_simulator,
                                                            target_maker=self.target_maker)
        ...
        if self.agent_type == 'basic':
            ...
            self.ag = FuturePredictorAgentAdvantage(self.sess,
                                                    agent_args)  # inital design: concat meas and img, then 2 branches for adv and val
        elif self.agent_type == 'advantage_nonorm':
            self.ag = FuturePredictorAgentAdvantageNoNorm(self.sess,
        ...
        elif self.agent_type == 'advantage_objects_depth':
            self.ag = FuturePredictorAgentAdvantageObjectDepth(self.sess, agent_args)
        ...
        _, self.test_objective_coeffs = my_util.make_objective_indices_and_coeffs(
            experiment_args['test_objective_coeffs_temporal'],
    ...
    def run(self, mode):
        ...
        if mode == 'show':
            ...
            self.ag.test_policy(self.multi_simulator, self.train_experience, self.test_objective_coeffs,
                                self.test_policy_num_steps, random_prob=self.test_random_prob, write_summary=False,
            ...
            if self.agent_type == 'advantage_objects':
                self.ag.train(self.multi_simulator, self.train_experience, self.num_train_iterations,
                              test_policy_experience=self.test_policy_experience, get_object=True)
            else:
                self.ag.train(self.multi_simulator, self.train_experience, self.num_train_iterations,
