...
class Agent:
    ...
    def __init__(self, sess, args):
        ...
        # TODO make magic number disappear
        self.state_objects_shape = (2, 2)
        ...
        self.object_for_net = [0, 1, 2, 3]
        self.object_for_manual = self.meas_for_manual
        self.target_dim = args['target_dim']
        self.target_names = args['target_names']
        self.discrete_controls = args['discrete_controls']
        self.discrete_controls_manual = args[
            'discrete_controls_manual']  # controls to be set "manually" (that is, by a user-specified function)
        ...
        self.fc_objects_params = args['fc_objects_params']
    ...
    def build_model(self):
        ...
        if self.postprocess_predictions:
            ...
            assert (
                np.all(abs(self.postprocess_predictions(self.preprocess_input_targets(test_arr)) - test_arr) < 1e-8))
        ...
        self.full_loss, self.errs_to_print, self.short_summary, self.detailed_summary = self.make_losses(
            self.pred_relevant, self.input_targets_preprocessed, self.objective_indices, self.objective_coeffs)
        ...
        # weight decay
        self.weights_norms = tf.reduce_sum(
            input_tensor=self.weight_decay * tf.stack([tf.nn.l2_loss(w) for w in self.t_vars]), name='weights_norm')
        self.loss_with_weight_decay = self.full_loss + self.weights_norms

        if self.clip_gradient:
            grads, grad_norm = tf.clip_by_global_norm(tf.gradients(self.loss_with_weight_decay, self.t_vars),
                                                      self.clip_gradient)
        ...
        self.detailed_summary = tf.summary.merge(
            self.detailed_summary + [tf.summary.scalar("learning rate", self.tf_learning_rate)])
        ...
        def reset_objective_coeffs(self, indices):
            ...
            if self.random_objective_coeffs:
                if self.agent.objective_coeffs_distribution == 'uniform_pos':
                    for ind in indices:
                        _, self.objective_coeffs[ind] = my_util.make_objective_indices_and_coeffs(
                            self.objective_coeffs_temporal, np.random.rand(self.num_objective_coeffs_meas))
                elif self.agent.objective_coeffs_distribution == 'uniform_pos_neg':
                    for ind in indices:
                        _, self.objective_coeffs[ind] = my_util.make_objective_indices_and_coeffs(
                            self.objective_coeffs_temporal, 2 * (np.random.rand(self.num_objective_coeffs_meas) - 0.5))
                else:
    ...
    def train(self, simulator, experience, num_steps, test_policy_experience=None, get_object=False):
        ...
        for _ in range(num_steps):
            if np.mod(self.curr_step, self.checkpoint_every) == 0:
                ...
                if np.mod(self.curr_step, self.checkpoint_every * 20) == 0:
                    self.save(self.checkpoint_dir + "/quarters_checkpoints", self.curr_step)
            ...
            self.train_one_batch(experience, get_object)
    ...
    def test_policy(self, simulator, experience, objective_coeffs, num_steps, random_prob=0., write_summary=False,
